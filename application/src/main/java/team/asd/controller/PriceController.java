...
public class PriceController {
	...
	@ApiOperation(value = "Get prices by date range for current product.",
			notes = "Require two different dates and productId as request parameters. Return list of prices witch date ranges are inside specified range as JSON objects. Parameters have to be in order 'start date, end date'.")
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Prices exist in table"),
			@ApiResponse(code = 404, message = "Custom message about error reasons or unknown error")
	})
